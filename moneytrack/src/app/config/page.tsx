"use client";
import MainLayout, { hammersmithOne, montserrat } from "../components/mainLayout";
...
export default function Config() {
  ...
  const handleSubmitForm = (data: UserForm) => {
    ...
    toast.promise(
      editUser(newUser, token as string).then((res) => {return res})
    , {
      loading: "Editando...",
        ...
        <div className="bg-green-600 w-[30vw] min-h-[50vh] p-2.5 flex flex-col justify-start items-center rounded-md">
          {isLoading ? (
            ...
            <div className="w-full h-full flex flex-col justify-start items-center gap-2.5">
              ...
              <h1 className={`${hammersmithOne.className} text-zinc-50 text-2xl text-center w-full p-2`}>
                Editar Perfil
              </h1>
              <form className="w-full flex flex-col justify-items-start gap-2.5 " onSubmit={handleSubmit((data) => console.log(data))}>
                <div className="w-full flex flex-col justify-around items-start gap-2.5 p-1.5">
                  <label htmlFor="name" className={`${montserrat.className} text-zinc-50 text-lg`}>Nome
                  ...
                  <Input
                    ...
                    {...register("name")}
                  />
                  <label htmlFor="username" className={`${montserrat.className} text-zinc-50 text-lg`}>Username {errors.username && <span className="text-red-500">{errors.username.message}</span>}</label>
                  <Input
                    id="username"
                    type="text"
                    width="w-full"
                    ...
                    {...register("username")}
                  />
                  <label htmlFor="email" className={`${montserrat.className} text-zinc-50 text-lg`}>Email {errors.email && <span className="text-red-500">{errors.email.message}</span>}</label>
                ...
                <div className="w-full flex justify-center items-center gap-2.5 ">
                  ...
                  <SecundaryButton width="w-full" content="Cancelar" onClick={() => reset()} />
                </div>

              </form>
              <PrimaryButton width="w-full" content="Solicitar Troca de Senha" />
              <PrimaryButton width="w-full border-2 border-red-500 bg-transparent text-red-500 hover:bg-red-500 hover:text-zinc-50 transition-colors durations-300 font-bold" content="Excluir Conta" />
            </div>
          )}
        </div>
      </div>
